...
class CompressionTest (BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        self.nodes.append(start_node(0, self.options.tmpdir, ["-debug", "-use-thinblocks=0"]))
        self.nodes.append(start_node(1, self.options.tmpdir, ["-debug", "-compressionlevel=1", "-use-thinblocks=0"]))
        ...
        self.nodes.append(start_node(0, self.options.tmpdir, ["-debug", "-compressionlevel=1", "-use-thinblocks=0"]))
        self.nodes.append(start_node(1, self.options.tmpdir, ["-debug", "-compressionlevel=2", "-use-thinblocks=0"]))
        self.nodes[0].generate(1)
        connect_nodes(self.nodes[0],1)
        self.sync_all()
        ...
        self.nodes.append(start_node(0, self.options.tmpdir, ["-debug", "-compressionlevel=2", "-use-thinblocks=0"]))
        self.nodes.append(start_node(1, self.options.tmpdir, ["-debug", "-compressionlevel=1", "-use-thinblocks=0"]))
        connect_nodes(self.nodes[0],1)
        self.nodes[0].generate(1)
        connect_nodes(self.nodes[0],1)
        ...
        self.nodes.append(start_node(0, self.options.tmpdir, ["-debug", "-compressionlevel=0", "-use-thinblocks=0"]))
        self.nodes.append(start_node(1, self.options.tmpdir, ["-debug", "-compressionlevel=1", "-use-thinblocks=0"]))
        connect_nodes(self.nodes[0],1)
        self.nodes[0].generate(1)
        connect_nodes(self.nodes[0],1)
        ...
        self.nodes.append(start_node(0, self.options.tmpdir, ["-debug", "-compressionlevel=2", "-use-thinblocks=0"]))
        self.nodes.append(start_node(1, self.options.tmpdir, ["-debug", "-compressionlevel=0", "-use-thinblocks=0"]))
        self.nodes[0].generate(1)
        connect_nodes(self.nodes[0],1)
        self.sync_all()
        self.nodes[0].sendtoaddress(self.nodes[1].getnewaddress(), 1)
        ...
        #sync compressionlevel:2 to compressionlevel:1 with Xthinblocks Enabled
        self.nodes.append(start_node(0, self.options.tmpdir, ["-debug", "-compressionlevel=2", "-use-thinblocks=1"]))
        self.nodes.append(start_node(1, self.options.tmpdir, ["-debug", "-compressionlevel=1", "-use-thinblocks=1"]))
        connect_nodes(self.nodes[0],1)
        self.nodes[0].generate(1)
        connect_nodes(self.nodes[0],1)
        self.sync_all()
        ...
        self.nodes[0].generate(1)
        self.sync_all()
        assert_equal(self.nodes[1].getbalance(), 7)
        stop_nodes(self.nodes)
        wait_bitcoinds()
