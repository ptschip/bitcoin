...
std::map<int64_t, std::pair<uint64_t, uint64_t> > CThinBlockStats::mapThinBlocks;
...
void CThinBlockStats::Update(uint64_t nThinBlockSize, uint64_t nOriginalBlockSize)
{
    ...
    CThinBlockStats::mapThinBlocks[GetTimeMillis()] = std::pair<uint64_t, uint64_t>(nThinBlockSize, nOriginalBlockSize);

    // Delete any entries that are more than 24 hours old
    int64_t nTimeCutoff = GetTimeMillis() - 60*60*24*1000;
    for (std::map<int64_t, std::pair<uint64_t, uint64_t> >::iterator mi = CThinBlockStats::mapThinBlocks.begin(); mi != CThinBlockStats::mapThinBlocks.end(); ++mi) {
        if ((*mi).first < nTimeCutoff)
            CThinBlockStats::mapThinBlocks.erase(mi);
    }
}
...
std::string CThinBlockStats::PercentToString()
{
    ...
    ss << "Compression (last 24hrs) is: " << nCompressionRate << "%";
    return ss.str();
}
